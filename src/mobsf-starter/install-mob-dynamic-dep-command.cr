  ...
  class InstallMobDynamicDepCommand
    ...
    def call(args : Array(String))
      ...
      {% if flag?(:darwin) %}
        ...
        fetchZipStatus = Process.run("sudo brew -y install zip unzip curl openjdk@11", shell: true, input: STDIN, output: STDOUT, error: STDERR)
        ...
        downloadAndroidCLStatus = Process.run("curl #{android_cl_mac_url} --output android_cl.zip", shell: true, input: STDIN, output: STDOUT, error: STDERR)
        ...
        unzipAndroidCLStatus = Process.run("unzip android_cl.zip && sudo rm -rf #{android_cl_mac_install_loc}/cmdline-tools/ && sudo mkdir --parents #{android_cl_mac_install_loc}/cmdline-tools/ && mv cmdline-tools tools && sudo mv tools #{android_cl_mac_install_loc}/cmdline-tools/", shell: true, input: STDIN, output: STDOUT, error: STDERR)
        ...
        downloadSDKDepStatus = Process.run("sudo #{android_cl_mac_install_loc}/cmdline-tools/tools/bin/sdkmanager platform-tools emulator", shell: true, input: STDIN, output: STDOUT, error: STDERR)
        ...
        Process.run("sed -i 's#export ANDROID_SDK_ROOT=#{android_cl_mac_install_loc}##' ~/.bashrc", shell: true, input: STDIN, output: STDOUT, error: STDERR)
        Process.run("sed -i 's#export ANDROID_HOME=#{android_cl_mac_install_loc}##' ~/.bashrc", shell: true, input: STDIN, output: STDOUT, error: STDERR)
        Process.run("sed -i 's#export MOBSF_ADB_BINARY=#{android_cl_mac_install_loc}/platform-tools/adb##' ~/.bashrc", shell: true, input: STDIN, output: STDOUT, error: STDERR)
        Process.run("sed -i 's#export PATH=$PATH:#{android_cl_mac_install_loc}/emulator:#{android_cl_mac_install_loc}/platform-tools:#{android_cl_mac_install_loc}/cmdline-tools/tools/bin##' ~/.bashrc", shell: true, input: STDIN, output: STDOUT, error: STDERR)

        Process.run("echo 'export ANDROID_SDK_ROOT=#{android_cl_mac_install_loc}' >> ~/.bashrc", shell: true, input: STDIN, output: STDOUT, error: STDERR)
        Process.run("echo 'export ANDROID_HOME=#{android_cl_mac_install_loc}' >> ~/.bashrc", shell: true, input: STDIN, output: STDOUT, error: STDERR)
        Process.run("echo 'export MOBSF_ADB_BINARY=#{android_cl_mac_install_loc}/platform-tools/adb' >> ~/.bashrc", shell: true, input: STDIN, output: STDOUT, error: STDERR)

        Process.run("echo 'export PATH=$PATH:#{android_cl_mac_install_loc}/emulator:#{android_cl_mac_install_loc}/platform-tools:#{android_cl_mac_install_loc}/cmdline-tools/tools/bin' >> ~/.bashrc", shell: true, input: STDIN, output: STDOUT, error: STDERR)
        ...
        downloadAndroidPkgStatus = Process.run(%[sudo #{android_cl_mac_install_loc}/cmdline-tools/tools/bin/sdkmanager "platforms;android-28" "system-images;android-28;default;x86" "build-tools;28.0.3" ], shell: true, input: STDIN, output: STDOUT, error: STDERR)
